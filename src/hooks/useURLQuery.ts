...
interface QueryConfig<T extends Record<string, any>> {
  defaults: T;
 ...
 * @example
 * ```typescript
 * const { getFiltersFromURL, updateURL, buildInitialURL, resetFilters } = useURLQuery({
 *   defaults: {
 *     period: '',
 ...
 * ```
 */
export const useURLQuery = <T extends Record<string, any>>(config: QueryConfig<T>) => {
  ...
  const hasActiveFilters = useCallback((): boolean => {
    ...
    return Object.keys(defaults).some(
      (key) => current[key as keyof T] !== defaults[key as keyof T],
    );
